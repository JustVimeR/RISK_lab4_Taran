# app.py (beautified, same-file design upgrade)
# --------------------------------------------
...
.topbar {{
  ...
  background: rgba(247,247,251,.9); backdrop-filter: saturate(180%) blur(6px);
...
.btn:hover {{ filter: brightness(1.02); transform: translateY(-0.5px); }}
.small {{ font-size: 12px; color: var(--muted); }}
.sticky-panel {{ position: sticky; top: 84px; }}
.kbd {{
  padding: 2px 6px; border-radius: 6px; border: 1px solid var(--border);
  background: #fff; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", monospace;
  font-size: 12px;
}}
.summary {{
...
.copy-inline {{ display:flex; gap:8px; align-items:center; }}
"""
...
app.layout = html.Div(
    className="container",
    children=[
        # Вставка глобальних стилів (без зовнішніх файлів)
        dcc.Markdown(f"<style>{GLOBAL_CSS}</style>", dangerously_allow_html=True),

        html.Div(className="topbar", children=[
            html.Div(className="title", children="Критерії прийняття рішень в умовах ризику — інтерактивне демо"),
            html.Div(className="subtitle", children="Редагуйте матрицю виграшів і ймовірності станів. Порівнюйте різні критерії та оцінюйте p за принципом максимальної ентропії."),
        ]),
                ...
                html.Div(className="card", children=[
                    ...
                    dash_table.DataTable(
                        ...
                        style_cell={
                            ...
                            "border": f"1px solid {BORDER}", "minWidth": 70
                        },
                        style_header={
                            "fontWeight": "700", "backgroundColor": ACCENT_SOFT,
                            "borderBottom": f"2px solid {ACCENT}"
                        },
                        style_data={"backgroundColor": "#fff"},
                        # style_data_conditional — оновлюється колбеком, щоб підсвічувати рекомендований рядок
                    ),
                    html.Div(className="actions", style={"display": "flex", "gap": "8px", "marginTop": "10px"}, children=[
                        html.Button("＋ Додати стратегію", id="add-row", n_clicks=0, className="btn ghost"),
                        html.Button("＋ Додати стан", id="add-col", n_clicks=0, className="btn ghost"),
                        html.Button("↺ Скинути", id="reset", n_clicks=0, className="btn secondary"),
                        html.Button("⤓ Експорт CSV", id="export", n_clicks=0, className="btn"),
                        dcc.Download(id="dl"),
                    ]),
                    html.Div(id="table-msg", className="small", style={"marginTop": "6px"}),
                ]),
            ]),
            # --------- Права панель ----------
            html.Div(style={"flex": 1}, children=[
                html.Div(className="card sticky-panel", children=[
                    html.H3("Ймовірності станів"),
                    html.Div(style={"display": "flex", "gap": "8px", "alignItems": "center", "marginBottom":"6px"}, children=[
                        html.Label("Режим оцінювання p:", className="small"),
                        dcc.Dropdown(
                            id="prob-mode",
                            options=[
                    ...
                    dash_table.DataTable(
                        ...
                        editable=True,
                        style_cell={"textAlign": "center", "padding": "6px", "border": f"1px solid {BORDER}"},
                        style_header={"fontWeight": "700", "backgroundColor": ACCENT_SOFT, "borderBottom": f"2px solid {ACCENT}"},
                        style_data={"backgroundColor": "#fff"},
                        # style_data_conditional — оновлюється колбеком, щоб підсвічувати max p
                    ),

                    html.Details(open=False, style={"marginTop": "8px"}, children=[
                        html.Summary("Налаштування MaxEnt (Гіббс–Джейнс)"),
                        html.Div(style={"display": "grid", "gridTemplateColumns": "1fr 1fr", "gap": "8px"}, children=[
                            html.Div(children=[
                                html.Label("Джерело m_j для обмеження", className="small"),
                                dcc.Dropdown(
                                    id="maxent-source",
                                    options=[
                            ...
                            html.Div(children=[
                                html.Label("Цільове очікування M", className="small"),
                                dcc.Slider(id="maxent-target", min=0, max=10, step=0.1, value=6.0,
                                           marks=None, tooltip={"always_visible": True}),
                            ]),
                        ]),
                        html.Div(id="maxent-hint", className="small", style={"marginTop": "4px"}),
                    ]),

                    html.Div(style={"marginTop": "10px"}, children=[
                        html.Label("Критерій рішення", className="small"),
                        dcc.Dropdown(
                            id="criterion",
                            options=[
                        ...
                        html.Div(style={"display": "flex", "gap": "12px", "alignItems": "center", "marginTop": "8px"}, children=[
                            html.Label("α для Гурвіца", className="small"),
                            dcc.Slider(id="alpha", min=0.0, max=1.0, step=0.05, value=0.6,
                                       marks=None, tooltip={"always_visible": True}),
                        ]),
                        html.Div(style={"display": "flex", "gap": "12px", "alignItems": "center", "marginTop": "8px"}, children=[
                            html.Label("γ для EV−γ·Var", className="small"),
                            dcc.Slider(id="gamma", min=0.0, max=1.0, step=0.05, value=0.1,
                                       marks=None, tooltip={"always_visible": True}),
                        ]),
                    ]),
                    html.Div(className="summary", id="summary", style={"marginTop": "12px", "fontWeight": "600"}),
                    html.Div(className="copy-inline", style={"marginTop":"6px"}, children=[
                        dcc.Clipboard(target_id="summary", title="Скопіювати підсумок", style={"cursor":"pointer"}),
                        html.Span("Скопіювати підсумок", className="small"),
                    ]),
                ]),
            ]),
        ]),
        ...
        html.Div(className="small", style={"textAlign":"center", "marginTop":"10px", "color":MUTED},
                 children="Tip: подвійний клік по легенді графіка — швидко сховати/показати всі серії"),
    ],
)
...
# ---------- Callback: графіки/підсумки + MaxEnt + підсвічування таблиць ----------
@callback(
    Output("payoff-heatmap", "figure"),
    Output("criterion-bars", "figure"),
    Output("hurwicz-sensitivity", "figure"),
    Output("summary", "children"),
    Output("prob-table", "data", allow_duplicate=True),
    Output("maxent-hint", "children"),
    Output("payoff-table", "style_data_conditional"),   # NEW: підсвіт рядка стратегії
    Output("prob-table", "style_data_conditional"),     # NEW: підсвіт комірки з max p
...
def update_graphs(p_data, p_cols, pr_data, prob_mode, maxent_source, maxent_target, crit, alpha, gamma):
    ...
    # --- графіки (єдиний стиль) ---
    base_layout = dict(template="plotly_white", margin=dict(l=40, r=20, t=50, b=40), hovermode="closest", font=dict(family="Inter"))
    heat = go.Figure(data=go.Heatmap(z=payoff, x=cols_ids, y=idx, colorbar=dict(title="Виграш")))
    heat.update_layout(title="Теплокарта матриці виграшів", **base_layout)
    ...
    bars.update_layout(barmode="group", title="Порівняння критеріїв", **base_layout)
    ...
    fig_sens.update_layout(title="Чутливість критерію Гурвіца до α", xaxis_title="α", yaxis_title="Значення", **base_layout)

    # --- підсумок
    summary = (
        f"✅ Обраний критерій: {chosen_name}. "
        f"Рекомендована стратегія: {idx[chosen_idx]}. "
    ...
    # --- стилі підсвічування таблиць
    # підсвічуємо рядок рекомендованої стратегії
    chosen_name = idx[chosen_idx]
    payoff_highlight = [
        {
            "if": {"filter_query": f'{{Стратегія}} = "{chosen_name}"'},
            "backgroundColor": ACCENT_SOFT,
            "border": f"1px solid {ACCENT}",
            "fontWeight": "700"
        }
    ]
    # підсвічуємо найбільшу ймовірність p_j
    max_p = float(np.max(probs)) if len(probs) else 0.0
    prob_highlight = []
    for i, c in enumerate(cols_ids):
        if abs(probs[i] - max_p) < 1e-12:
            prob_highlight.append({
                "if": {"column_id": c},
                "backgroundColor": ACCENT_SOFT,
                "border": f"1px solid {ACCENT}",
                "fontWeight": "700"
    ...
    return heat, bars, fig_sens, summary, prob_row, maxent_msg, payoff_highlight, prob_highlight
